...
import Model.NMF_attention_inter as NMF
import pickle
import sys
import atexit
        ...
        if epoch % verbose == 0:
            ...
            if (hr + ndcg) / 2. > (best_hr + best_ndcg) / 2.:
                ...
                if args.out > 0:
                    ...
                    #                              (args.dataset))
                    save_path = saver_model.save(sess, "/Dataset/weiyu/Pretrain/attention_self_%s_inter1_0110%s" %
                                                 (args.dataset, args.sign))
