...
class Model:
    ...
    def predict(self):
        ...
        u_x = tf.contrib.layers.fully_connected(u_x, 64, tf.identity,
                                              weights_initializer=tf.contrib.layers.xavier_initializer(),
                                              biases_initializer=tf.contrib.layers.xavier_initializer())
        i_x = tf.contrib.layers.fully_connected(item_ratings, 300,
                                              weights_initializer=tf.contrib.layers.xavier_initializer(),
                                              biases_initializer=tf.contrib.layers.xavier_initializer())
        i_x = tf.contrib.layers.fully_connected(i_x, 64, tf.identity,
                                              weights_initializer=tf.contrib.layers.xavier_initializer(),
                                              biases_initializer=tf.contrib.layers.xavier_initializer())
        # weights_initializer=tf.truncated_normal_initializer(stddev=0.01))
        fen_zhi = tf.reduce_sum(u_x * i_x, 1, keep_dims=True)
